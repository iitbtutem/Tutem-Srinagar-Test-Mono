...
function Textarea({
  ...
  ...props
}: React.ComponentProps<typeof TextInput> & { className?: string; placeholderClassName?: string }) {
  return (
    <TextInput
      className={cn(
        'text-foreground border-primary dark:border-foreground/60 dark:bg-input/30 flex min-h-16 w-full flex-row rounded-2xl border bg-transparent px-3 py-2 text-base shadow-sm shadow-black/5 md:text-sm',
        Platform.select({
