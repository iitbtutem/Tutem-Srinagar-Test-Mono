...
function Timeline({ ride }: { ride: Ride }) {
  const abortRideReason = ride.rideReasons.find(reason => reason.driverId === ride.driverId || reason.driverId === undefined)
  const steps = [
    ...
    ...abortRideReason ? [{ status: 'Abort', ts: abortRideReason._creationTime, done: true }] : [],
  ...
  <View className="mb-3 gap-2">
    ...
    <View className="self-start rounded-full bg-destructive/15 px-3 py-1 max-w-full">
      <Text
        numberOfLines={2}
